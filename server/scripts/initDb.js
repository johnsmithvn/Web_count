...
db.serialize(() => {
  ...
  `, (err) => {
    if (err) console.error('Error creating scans table:', err);
    else console.log('✅ Scans table created successfully');
  });
  ...
  `, (err) => {
    ...
    else console.log('✅ Folders table created successfully');
  });
  ...
  // Create indexes (all run sequentially inside serialize)
  const indexes = [
  ...
  indexes.forEach((index) => {
    db.run(index.sql, (err) => {
      if (err) console.error(`❌ Error creating index ${index.name}:`, err);
      else console.log(`✅ Index ${index.name} created successfully`);
    });
  });
});

// Create admin user after serialize block completes
// bcrypt.hash is async so we handle it with a callback chain
db.serialize(() => {
  console.log('Creating admin user...');

  // Use a wrapper to handle bcrypt async inside serialize
  db.run('SELECT 1', () => {
    bcrypt.hash('admin', 10, (err, hash) => {
      ...
      `, ['admin', hash, new Date().toISOString()], (err) => {
        ...
        else console.log('✅ Admin user created: admin/admin');
        finalize();
      });
    });
  });
